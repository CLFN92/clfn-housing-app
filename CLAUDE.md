...
- **Module gating** — `ai_assistant` is registered in `CLFN_MODULES` (`_enabled`/`_licensed`), so it auto-surfaces a Settings → Nation → Modules toggle (label "AI Assistant (Chat + Draft Notes)") and is per-nation licensable. `ai-assistant.js` is loaded on `housing.html` + `inspections.html`.
