...
### Housing Assistant (natural-language staff chat, `ai-assistant.js` + `supabase/functions/ai-assistant/`)
A read-only conversational assistant for **staff** (Phase AI in `PLAN.md`). It does two things: (1) answers **data** questions ("how many vacant units?", "list applications awaiting review") by letting Claude call a single whitelisted, read-only `query_database` tool, and (2) answers **how-to / procedural** questions ("how do I complete a maintenance request?", "how do I do an application?") from a static `HOW_TO` knowledge block baked into the Edge Function's system prompt, tailored to the caller's role. The `HOW_TO` block is the canonical place to add/adjust workflow guidance — keep it ASCII and grounded in the real UI. The client widget's starter chips are **role-aware** (`SUGGEST_BY_ROLE` in `ai-assistant.js`, keyed off `window.currentRole`) and rendered on open.

- **Edge Function `supabase/functions/ai-assistant/index.ts`** — same security spine as `send-notification`: verifies the caller's Supabase JWT, then resolves their role from the `staff` table (`is_active=true`; rejects non-staff with 403). Runs a Claude **tool-use loop** (`https://api.anthropic.com/v1/messages`, model `ANTHROPIC_MODEL` default `claude-sonnet-4-6`). The **Anthropic key (`ANTHROPIC_API_KEY`) lives only in function secrets** — never in the browser. The one tool, `query_database`, reads via the service-role key but is gated by a **per-table role allowlist** (`TABLES` map) + forced row filters: field employees are constrained to their own `assigned_team=in_house` SOWs; `finance_*` tables are ED/CFO/Finance only; `applications` are hidden from field employees. Hard caps: `MAX_ROW_LIMIT=50`, `MAX_TOOL_TURNS=6`. **No write tools exist** — the assistant cannot mutate data. Filter ops are whitelisted (`SAFE_OPS`); table/column names are regex-validated. **ASCII-only source** (same rule as send-notification — verify with `grep -P "[^\x00-\x7F]"`). Deploy manually + set `ANTHROPIC_API_KEY`; no client CSP change (browser only calls the Supabase functions host, Anthropic call is server-side).
- **Client widget `ai-assistant.js`** — self-contained IIFE (mirrors `reno-questionnaire.js`): a floating button + chat panel that injects its own scoped `<style>`, keeps `history[]`, and POSTs `{messages}` to `/functions/v1/ai-assistant` with `HOUSING_SESSION.accessToken`. Renders a safe markdown subset (escape-first, then bold/code/tables). Mounts only when `signedIn()` AND `moduleOn('ai_assistant')`. `window.openHousingAssistant()` / `closeHousingAssistant()`. Loaded on all 8 staff pages (after `shared-data.js`).
- **Module gating** — `ai_assistant` is registered in `CLFN_MODULES._enabled`/`_licensed`, so it auto-surfaces a Settings → Nation → Modules toggle and is per-nation licensable.
