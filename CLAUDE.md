...
- **Data model is context-stuffing PLUS a read-only query tool.** The client (`ai-assistant.js`) gathers app/unit/SOW/RFQ/contractor/reno-progress data from in-memory globals (`window.applications`, `housingUnits`, `_sowCache`, `_rfqCache`, `_contractors`, `_renoProgress`), trims/flattens it, and POSTs it as `context` to `ai-chat`. For quick questions Claude answers from that context; for **exact counts / complete lists / records not loaded on the current page**, chat mode also exposes a read-only **`query_database`** tool (tool-use loop, chat mode only — `draft` mode is a single call). The tool reads via the service-role key but is gated by a **per-table role allowlist** (`TABLES`) + forced row filters (field employees constrained to their own `assigned_team=in_house` SOWs; `finance_*` ED/CFO/Finance only; `applications`/`housing_rfq` hidden from field employees; `housing_audit_log`/`staff` ED/HM only), with hard caps (`MAX_ROW_LIMIT=50`, `MAX_TOOL_TURNS=6`) and whitelisted filter ops (`SAFE_OPS`). **No write tools exist.** The **how-to guidance lives in the `HOW_TO` block** in `ai-chat/index.ts` — the canonical place to add/adjust workflow instructions (keep it ASCII, grounded in the real UI).
