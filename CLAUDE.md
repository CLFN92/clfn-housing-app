...
- **Pending wiring:** per-action approval events (`mgr_approved`, `hm_approved`, `ed_approved`, `declined`, `returned` for applications; `hm_recommended`, `approved`, `declined`, `returned` for contractors). TODO comments mark the firing points in housing-modals.js (application actions) and shared-data.js (contractor actions).
- **Applicant PDF flow:** `_captureApplicationPreviewHtml` temporarily intercepts `showPrintPanel` so the existing 320-line `printApplicationPreview()` HTML doc is captured without showing the on-screen panel. Mounted in an off-screen iframe (`794px × 2400px`, A4-equivalent). `jsPDF.html()` renders via html2canvas — both lazy-loaded from CDN via `_loadJsPdf` / `_loadHtml2Canvas`. PDF bytes extracted via `doc.output('datauristring')` + strip prefix to get clean base64. **Do NOT use `btoa(doc.output())`** — has encoding edge cases with arbitrary PDF bytes.
