...
- **Data model is context-stuffing, not server-side query.** The client (`ai-assistant.js`) gathers app/unit/SOW/RFQ/contractor/reno-progress data from in-memory globals (`window.applications`, `housingUnits`, `_sowCache`, `_rfqCache`, `_contractors`, `_renoProgress`), trims/flattens it, and POSTs it as `context` to `ai-chat`. The function does **not** read housing tables itself — it builds the system prompt from that context and calls Claude (`claude-sonnet-4-6`, no tools). The **how-to guidance lives in the `HOW_TO` block** in `ai-chat/index.ts` — the canonical place to add/adjust workflow instructions (keep it ASCII, grounded in the real UI).
