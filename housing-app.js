...
function renderDashTable(){
  ...
  tbody.innerHTML=filtered.map(function(a){
    ...
    // "Who acts next" hint shown under the status pill so the hand-off is explicit.
    var _nextStepMap={draft:'Finish & submit',submitted:'→ Housing Manager review',file_update:'→ Housing Manager review',mgr_approved:'→ Executive Director approval',hm_approved:'Approved (file update)',ed_approved:'→ Match to a unit',returned:'→ Applicant to resubmit',assigned:'→ Generate agreement',housed:'',declined:''};
    var _nextStep=(a.appType==='commercial')?(a.status==='ed_approved'?'→ Assign to a building':(a.status==='submitted'?'→ Review':'')):(_nextStepMap[a.status]||'');
    ...
    return '<tr>'
      ...
      +'<td><span class="pill '+sp[1]+'"><span class="pill-dot"></span>'+sp[0]+'</span>'+(_nextStep?'<div style="font-size:10px;color:var(--muted);margin-top:3px;white-space:nowrap;">'+escapeHtml(_nextStep)+'</div>':'')+'</td>'
